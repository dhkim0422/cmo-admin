 ...
 */
package com.insilicogen.common.jfile.view;
...
import com.insilicogen.common.jfile.utils.SpringUtils;
import com.insilicogen.common.jfile.utils.ZipUtils;
...
//		setContentType("application/octet-stream");
	...
	protected void renderMergedOutputModel(@SuppressWarnings("rawtypes") Map model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(obj != null && obj.getClass().isArray()) {
			...
			setResponseHeader(request, response, "압축.zip", -1);
			ZipUtils.createZipJFile(files, response.getOutputStream());
		}else{
			File file = (File)obj;
			setResponseHeader(request, response, (file instanceof JFile) ? ((JFile)file).getOriginalFileNm() : file.getName(), (int)file.length());
	 ...
	 */
	private void setResponseHeader(HttpServletRequest request, HttpServletResponse response, String fileNm, int contentLength) throws UnsupportedEncodingException {
		...
		if (Agent.indexOf("MSIE") >= 0) {
			...
			if (IEV.equalsIgnoreCase("5.5")) {
				response.setHeader("Content-Disposition", "filename=\"" +  getEncoding(fileNm)+"\"");
			} else {
				response.setHeader("Content-Disposition", "attachment;filename=\"" + getEncoding(fileNm)+"\"");
			}
		} else {
			response.setHeader("Content-Disposition", "attachment;filename=\"" + getEncoding(fileNm)+"\"");
		}
	...
	private String getEncoding(String fileNm)
			throws UnsupportedEncodingException {
		return new String(fileNm.getBytes("euc-kr"), "8859_1");
	}
}