	            ...
	            <div class="card-body">
	                   	...
	                   	<div class="col-md-12">
	                		...
	                		<div class="table-responsive">
			                	<table id="groupCodeTable" class="table table-hover display no-wrap custom-table">
			                		<thead>
                                        <tr>
                                            <th scope="col"><label class="form-control-label" for="groupCd">그룹코드</label></th>
                                    ...
                                    <tbody>
                                    	<tr>
                                    		<td><div class="form-group"><form:input type="text" class="form-control" path="groupCd" readonly="true"/></div></td>
                                    		...
                                    		<td>
                                    			<div class="form-group">
		                                    		<form:select class="custom-select" path="useAt">
	            	...
	            	<div class="row">
	                   	<div class="col-md-12">
			                ...
			                <div class="table-responsive">
			                	<table id="codeTable" class="table table-hover display no-wrap custom-table">
                                    <tbody>
                                    	<c:forEach items="${data.codeList}" var="list" varStatus="status">
                                    	<tr>
                                    		<td scope="row"><c:out value="${list.oriCode}" /></td>
                                    		<td><c:out value="${list.code}" /></td>
                                    		<td><c:out value="${list.codeNm}" /></td>
                                    		<td><c:out value="${list.codeNmEn}" /></td>
                                    		<td><c:out value="${list.colOrd}" /></td>
                                    		<td><c:out value="${list.useAt}" /></td>
                                    		<td><c:out value="${list.codeDc}" /></td>
                                    		<td><c:out value="${list.codeIcon}" /></td>
                                    	</tr>
                                    	</c:forEach>
                                    </tbody>
			                	</table>
...
<script type="text/javaScript" language="javascript">
	...
	function fnSave(){
		var url = '<c:url value="/common/code/updateCode.json"/>';
	...
	var codeTable = $('#codeTable').DataTable({
		...
		columnDefs : [
			{ targets: [0], searchable: false, orderable: false, defaultContent: "", className: 'select-checkbox', render: function (data, type, row, meta) { return fnColumnRender(data, meta, 'hidden')}},
	...
	function fnRemoveRow(){
		...
		if(selectedRows.length==0){
			...
			$.each(selectedRows, function(index, item){
				$("#delList").append('<input type="hidden" class="form-control" name="delList" value="'+((item && item.oriCode) || '')+'" />');
			});
